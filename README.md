...
This line graph displays the number of bikes that are checked out by duration for each gender by the hour, and the graph can be filtered by the hour and gender. First hour of the trip female gender had the highest number of trips, followed by male then unknowns. The following hours has similar number of bike trips for all genders. With this information we could target the non female genders and find strategies to increase the number of bike trips for them. Another strategy should be done to increase the number of bikes after the first hour for all genders. 
...
#### 1. What Are Peak Riding Hours in the Month of August
...
#### 2. Which Bikes Are Most Likely Due for Repair? 
...
#### 5. User Trips by Gender by Weekday
A heatmap is created showing the number of bike trips for each type of user and gender for each day of the week, and you can filter by user and gender. It is clear that the Thursdays are the female subscribers day and the most trips are made on Thursdays by female subscribers.
### Additional visualizations

![This image shows the trend of starting hour for different user types](images/gender-user.PNG)

#### 1.What is the trend of user types amongst different genders
...
#### 2.What is the trend of starting hour for different user types
